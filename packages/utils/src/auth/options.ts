...
const handler: NextAuthOptions = {
    providers: [

        {
            ...
            authorization: { 
                ...
                params: { 
                    ...
                    code_challenge: process.env.LICHESS_CODE_CHALLENGE as string,  
        ...
        CredentialsProvider({
            ...
            async authorize(credentials) {
                const res = await fetch(`${BACKEND_API_URL}/auth/registration/`, {
                    method: 'POST',
                    body: JSON.stringify(credentials),
                    headers: { 'Content-Type': 'application/json' },
                });
    ...
    callbacks: {
        ...
        async jwt({ token, user, account }: { token: JWT; account: Account | null; user: User | AdapterUser }) {
            if (account) {
                ...
                if (provider_access_token) {
                    const providerAuth = await fetch(`${BACKEND_API_URL}/auth/${account.provider}/`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
                        body: JSON.stringify({ access_token: provider_access_token, id_token: account.id_token }),
                    });
            ...
            if (verifyData.success != null && verifyData.success === false) {
                ...
                const refreshRes = await fetch(`${BACKEND_API_URL}/auth/token/refresh/`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
